#!/usr/bin/env bash
# GLM Tray Install Script (Linux + macOS)
# Usage: curl -fsSL https://raw.githubusercontent.com/kiwina/glm-tray/main/install.sh | bash
#
# Environment variables:
#   VERSION     - Install specific version (e.g., "0.1.0"), default: latest
...
REPO="kiwina/glm-tray"
APP_NAME="GLM Tray"
APP_ID="com.kiwina.glm-tray"
...
get_version() {
    ...
    local response
    if response=$(curl -fsSL -H "User-Agent: glm-tray-installer" "${GITHUB_API}/latest" 2>/dev/null); then
...
build_download_url() {
    ...
    case "$PLATFORM" in
        linux)
            case "$PKG_EXT" in
                deb)
                    DOWNLOAD_URL="${base_url}/glm-tray_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    FILENAME="glm-tray_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    ;;
                rpm)
                    DOWNLOAD_URL="${base_url}/glm-tray-${RELEASE_VERSION}-1.${RPM_ARCH}.rpm"
                    FILENAME="glm-tray-${RELEASE_VERSION}-1.${RPM_ARCH}.rpm"
                    ;;
                AppImage)
                    DOWNLOAD_URL="${base_url}/glm-tray_${RELEASE_VERSION}_${DEB_ARCH}.AppImage"
                    FILENAME="glm-tray_${RELEASE_VERSION}_${DEB_ARCH}.AppImage"
                    ;;
            esac
            ;;
        macos)
            # Use arch-specific DMG
            if [[ "$ARCH_LABEL" == "aarch64" ]]; then
                DOWNLOAD_URL="${base_url}/glm-tray_${RELEASE_VERSION}_aarch64.dmg"
                FILENAME="glm-tray_${RELEASE_VERSION}_aarch64.dmg"
            else
                DOWNLOAD_URL="${base_url}/glm-tray_${RELEASE_VERSION}_x64.dmg"
                FILENAME="glm-tray_${RELEASE_VERSION}_x64.dmg"
            fi
            ;;
    esac
...
install_linux() {
    ...
    case "$PKG_MANAGER" in
        ...
        appimage)
            ...
            run cp "$DOWNLOAD_PATH" "${install_dir}/glm-tray"

            if [[ ":$PATH:" != *":${install_dir}:"* ]]; then
                warn "Add ${install_dir} to your PATH to run glm-tray from anywhere"
